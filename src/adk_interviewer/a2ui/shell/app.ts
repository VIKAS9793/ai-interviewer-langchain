...
import { config as interviewerConfig } from "./configs/interviewer.js";
...
export class A2UILayoutEditor extends SignalWatcher(LitElement) {
  ...
  connectedCallback() {
    ...
    const appKey = urlParams.get("app") || "interviewer";
    this.config = configs[appKey] || configs.interviewer;
  ...
  render() {
    return [
      ...
      this.#maybeRenderConversationInput(),  // v4.7.1: Continue conversation
  ...
  // v4.7.1: Render input for follow-up messages after surfaces are shown
  #maybeRenderConversationInput() {
    if (this.#requesting) return nothing;
    if (this.#lastMessages.length === 0) return nothing;  // Only show when has messages
    ...
    >
      <div>
        <input
          required
          placeholder="${this.config.placeholder || "Continue the conversation..."}"
          autocomplete="off"
